...
class MRMostUsedWord(MRJob):
    ...
    def mapper_count_words_init(self):
        self.whole_page = ""

    def mapper_count_words(self, _, line):
        self.whole_page += line

    def mapper_count_words_final(self):
        soup = BeautifulSoup(self.whole_page,'lxml')
        pages = soup.find_all('page')
        for page in pages:
            root = etree.fromstring(str(page))
            a    = etree.tostring(root,encoding='unicode',method="text")
            self.whole_page = ""
            for word in WORD_RE.findall(a):
                yield (word.lower(), 1)
    ...
    def steps(self):
        return [
            MRStep(mapper_init=self.mapper_count_words_init,
                   mapper=self.mapper_count_words,
                   mapper_final=self.mapper_count_words_final,
